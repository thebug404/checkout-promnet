 ...
 * @param {object} pspCredential - The merchant_psp_credential record
 */
 ...
 *
 * @param {object} pspCredential - The merchant's PSP credential record
 * @param {object} requestPayload - The capture context request body
 * @returns {Promise<{captureContext: string, decodedData: object}>}
 */
export function generateCaptureContext(pspCredential, requestPayload) {
  return new Promise((resolve, reject) => {
    ...
    instance.generateUnifiedCheckoutCaptureContext(requestPayload, (error, data, response) => {
      ...
      // data is a JWT string: header.payload.signature
      try {
 ...
 *
 * @param {object} pspCredential - The merchant's PSP credential record
 * @param {object} paymentRequest - { transientToken, orderInformation }
 * @returns {Promise<object>} The payment response from CyberSource
 */
